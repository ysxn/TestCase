...
/**
 * Create an {@link ImageLoader}. Here defines two {@link ImageLoader}, one is
 * the default, the other one is the stable. You can use the stable one to load
 * the images which will not change frequently.
 * 
...
public class ImageLoaderFactory {
     ...
     * 
     * @param context
     * @param memoryCacheSizeInKB How many memory should use. Will not be
     *            greater than 50% of free memory
     ...
     * @param memoryCacheSizeInKB How many memory should use. Will not be
     *            greater than 50% of free memory
     * @param defaultDiskCachePath Default image cache path. Absolute path or a
     *            relative path under cache directory. External cache first. If
     *            not specified, using {@link #DEFAULT_FILE_CACHE_DIR} under
     *            cache directory.
     * @param defaultDiskCacheSizeInKB Default disk cache size.
     */
    public static void customizeCache(Context context, int memoryCacheSizeInKB,
            String defaultDiskCachePath, int defaultDiskCacheSizeInKB) {
        customizeCache(context, memoryCacheSizeInKB, defaultDiskCachePath,
                defaultDiskCacheSizeInKB, null, 0);
    }
     ...
     */
    public static void customizeCache(Context context, int memoryCacheSizeInKB,
            int defaultDiskCacheSizeInKB, int stableDiskCacheSizeInKB) {
        customizeCache(context, memoryCacheSizeInKB, null, defaultDiskCacheSizeInKB, null,
     ...
     *            cache directory.
     * @param defaultDiskCacheSizeInKB Default disk cache size.
     * @param stableDiskCachePath Path for stable cache directory. Default is
     *            {@link #STABLE_FILE_CACHE_DIR}
     * @param stableDiskCacheSizeInKB Stable disk cache size.
     */
    public static void customizeCache(Context context, int memoryCacheSizeInKB,
            String defaultDiskCachePath, int defaultDiskCacheSizeInKB, String stableDiskCachePath,
        ...
        if (defaultDiskCacheSizeInKB > 0 && !TextUtils.isEmpty(defaultDiskCachePath)) {
            ImageDiskCacheProvider imageFileProvider = getImageFileProvider(context,
                    defaultDiskCachePath, defaultDiskCacheSizeInKB, DEFAULT_FILE_CACHE_DIR);
            if (imageFileProvider != null) {
                sDefaultImageProvider = new ImageProvider(context, getDefaultImageMemoryCache(),
                        imageFileProvider);
            }
        }
        ...
        if (stableDiskCacheSizeInKB > 0 && !TextUtils.isEmpty(stableDiskCachePath)) {
            ImageDiskCacheProvider imageFileProvider = getImageFileProvider(context,
                    stableDiskCachePath, stableDiskCacheSizeInKB, STABLE_FILE_CACHE_DIR);
            if (imageFileProvider != null) {
                sStableImageProvider = new ImageProvider(context, getDefaultImageMemoryCache(),
                        imageFileProvider);
            }
        }
    }

    private static ImageDiskCacheProvider getImageFileProvider(Context context, String path,
            int sizeInKB, String fallbackCachePath) {
        if (sizeInKB <= 0) {
            sizeInKB = DEFAULT_FILE_CACHE_SIZE_IN_KB;
        }

        DiskFileUtils.CacheDirInfo dirInfo = DiskFileUtils.getDiskCacheDir(context, path, sizeInKB,
                fallbackCachePath);

        ImageDiskCacheProvider imageFileProvider = ImageDiskCacheProvider.createLru(
                dirInfo.realSize, dirInfo.path);
    ...
    private static ImageLoader createInner(Context context, ImageProvider imageProvider,
            ImageLoadHandler imageLoadHandler) {
        return create(context, imageProvider, sDefaultImageTaskExecutor, sDefaultImageReSizer,
                imageLoadHandler);
    }

    private static ImageLoader create(Context context, ImageProvider imageProvider,
        ...
        ImageLoader imageLoader = new ImageLoader(context, imageProvider, imageTaskExecutor,
                imageReSizer, imageLoadHandler);
    ...
    public static ImageProvider getDefaultImageProvider(Context context) {
        if (null == sDefaultImageProvider) {
            ImageDiskCacheProvider imageFileProvider = getImageFileProvider(context, null, 0,
