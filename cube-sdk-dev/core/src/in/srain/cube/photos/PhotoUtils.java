...
public class PhotoUtils {
    ...
    public static void toCamera(Activity activity, File outputFile, int requestCode) {
        ...
        if (resolveInfoList == null && resolveInfoList.size() == 0) {
            Toast.makeText(activity, R.string.cube_photo_can_not_use_camera, Toast.LENGTH_LONG)
                    .show();
            return;
        }
    ...
    public static void toAlbum(Activity activity, int requestCode) {
        try {
            ...
            Toast toast = Toast.makeText(activity, R.string.cube_photo_can_not_open_album,
                    Toast.LENGTH_SHORT);
    ...
    public static void toCrop(Activity activity, File file, File outputFile, CropOption info,
            int requestCode) {
        ...
        try {
            if (Build.VERSION.SDK_INT == 19 && DocumentsContract.isDocumentUri(activity, uri)) {
                ...
                cursor = activity.getContentResolver().query(
                        MediaStore.Images.Media.EXTERNAL_CONTENT_URI, columns, idWhere,
                        new String[] {
                            id
                        }, null);
            } else {
