    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (alarm.mId == 1) {
            ...
            if (isInCall /*|| isAlarmBoot()*/) {
                /**lenovo-sw linrk1 modify begin 2014-05-06 ,add Notification**/
                if(isInCall){
                    NotificationManager manager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);
                    Notification notification = new Notification();
                    notification.icon = R.drawable.ic_settings_schpwroff;
                    notification.tickerText = context.getString(R.string.schedule_power_off_time_out_summary);
                    notification.defaults=Notification.DEFAULT_SOUND;
                    notification.audioStreamType= android.media.AudioManager.ADJUST_LOWER;
					notification.flags |= Notification.FLAG_AUTO_CANCEL;
                    Intent isCallIntent = new Intent(context,AlarmClock.class);
                    PendingIntent pendingIntent = PendingIntent.getActivity(context, 0, isCallIntent,0);
                    notification.setLatestEventInfo(context, context.getString(R.string.schedule_power_off_time_out_title),context.getString(R.string.schedule_power_off_time_out_summary), pendingIntent);
                    manager.notify(1, notification);
                }
                /**lenovo-sw linrk1 modify end 2014-05-06 ,add Notification**/
