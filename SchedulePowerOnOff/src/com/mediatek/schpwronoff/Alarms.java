...
public class Alarms {
    ...
    /*Lenovo-sw wuzb1 add 2013-09-11 add time invalidation check */
    static final int POWER_TIME_INTERVAL = 10;
    ...
    public static void setAlarm(Context context, int id, boolean enabled, int hour, int minutes,
        ...
        Log.d(TAG, "**  setAlarm * idx " + id + " hour " + hour + " minutes " + minutes + " enabled " + enabled + " time "
                + time);
    ...
    public static Alarm calculateNextAlert(final Context context, final int alarmId) {
        ...
        Log.d(TAG, "Alarms.calculateNextAlert()_now = " + now);
        try {
    ...
    private static void enableAlert(Context context, final Alarm alarm, final long atTimeInMillis) {
        ...
        /*Lenovo-sw wuzb1 2014-01-27 add begin, set flag for broadcast enforce */
        intent.setFlags(Intent.FLAG_RECEIVER_FOREGROUND);
        /*Lenovo-sw wuzb1 2014-01-27 add end, set flag for broadcast enforce */
        ...
        /**Lenovo-sw linrk1 modify begin 2014-10-13  **/
        if(isKitKatOrLater()){
            am.setExact(0, atTimeInMillis, sender);
        }else{
            am.set(0, atTimeInMillis, sender);
        }
        /**Lenovo-sw linrk1 modify end 2014-10-13  **/
    ...
    private static void enableAlertPowerOn(Context context, final Alarm alarm, final long atTimeInMillis) {
        ...
        if(android.os.Build.HARDWARE.equals("qcom")){
            /**Lenovo-sw linrk1 modify begin 2014-10-16  **/
             if(isKitKatOrLater()){
                am.setExact(6, atTimeInMillis, sender);
              }else{
                am.set(6, atTimeInMillis, sender);
              }
            /**Lenovo-sw linrk1 modify end 2014-10-16  **/
        }else {
            if(isKitKatOrLater()){
        ...
        // String timeString = formatDayAndTime(context, c);
        // saveNextAlarm(context, timeString);
        /*Lenovo-sw wuzb1 add 2013-07-31 begin, add alarm clock warning on shutdown dialog*/
        String str;
        if (Locale.getDefault().getLanguage().equals("ru")) {
            str = formatDayAndTime(context, c).toUpperCase();
        }
        else {
            str = formatDayAndTime(context, c);
        }
        Settings.System.putString(context.getContentResolver(),
            Settings.System.NEXT_ALARM_FORMATTED+"_poweron", str);
        /*Lenovo-sw wuzb1 add 2013-07-31 end, add alarm clock warning  on shutdown dialog*/
    }
    ...
    static void disableAlertPowerOn(Context context) {
        ...
        /*Lenovo-sw wuzb1 add 2013-07-31 begin, add alarm clock warning on shutdown dialog*/
        Settings.System.putString(context.getContentResolver(),
            Settings.System.NEXT_ALARM_FORMATTED+"_poweron", "");
        /*Lenovo-sw wuzb1 add 2013-07-31 end, add alarm clock warning on shutdown dialog*/
    }
    ...
    private static String formatDayAndTime(final Context context, Calendar c) {
        String format = get24HourMode(context) ? getDay24ModeFormat().toString() : getDay12ModeFormatWithAmPm().toString();
        return (c == null) ? "" : (String) DateFormat.format(format, c);
    }
    ...
    /*Lenovo-sw wuzb1 add 2013-10-10 begin, add time invalidation check */
    static int getDayOfWeekIndex(int saveMinuteTime){
        Calendar todayDate = Calendar.getInstance();
        int nowMinute = todayDate.get(Calendar.HOUR_OF_DAY)*60
                + todayDate.get(Calendar.MINUTE);
        if(nowMinute>=saveMinuteTime){
            todayDate.add(Calendar.DATE, 1);
        }
        int dayIndex = (todayDate.get(Calendar.DAY_OF_WEEK) + 5) % 7;
        return dayIndex;
    }

    static boolean isDaySet( Alarm.DaysOfWeek daysOfWeek , int dayIndex){
        dayIndex=dayIndex%7;
        return (daysOfWeek.getCoded() & (1 << dayIndex)) != 0;
    }

    static void calculateCompareAlarmTime(int[] saveTime, int saveMinuteTime, Alarm.DaysOfWeek saveDays,
            int[] settedTime, int settedMinuteTime, Alarm.DaysOfWeek settedDays){
        for(int index=0; index<saveTime.length; index++){
            if(isDaySet(saveDays,index)){
                saveTime[index]=saveMinuteTime;
            }else{
                saveTime[index]=-1;
            }
            if(isDaySet(settedDays,index)){
                settedTime[index]=settedMinuteTime;
            }else{
                settedTime[index]=-1;
            }
        }
        if(!saveDays.isRepeatSet()){
            saveTime[getDayOfWeekIndex(saveMinuteTime)]=saveMinuteTime;
        }
        if(!settedDays.isRepeatSet()){
            settedTime[getDayOfWeekIndex(settedMinuteTime)]=settedMinuteTime;
        }
    }

    static boolean compareAlarmTime(int[] saveTime, int[] settedTime){
        for(int dayIndex=0; dayIndex<saveTime.length; dayIndex++){
            if(saveTime[dayIndex] < 0){
                continue;
            }
            //compare today with today-1
            if(dayIndex-1 >= 0 && settedTime[dayIndex-1] >= 0 &&
                (24*60-settedTime[dayIndex-1]+saveTime[dayIndex]) < POWER_TIME_INTERVAL){
                return false;
            }
            //compare today with today
            if(settedTime[dayIndex]>=0 &&
                Math.abs(saveTime[dayIndex] - settedTime[dayIndex]) < POWER_TIME_INTERVAL){
                return false;
            }
            //compare today with today+1
            if(dayIndex+1 <saveTime.length && settedTime[dayIndex+1]>=0 &&
                (24*60-saveTime[dayIndex] +settedTime[dayIndex+1]) < POWER_TIME_INTERVAL){
                return false;
            }
        }
        return true;
    ...
    static boolean isAlarmTimeAllowed(Context context, int saveId, boolean saveEnable,
        int saveHour, int saveMinute, Alarm.DaysOfWeek saveDays) {
        if (saveEnable == false) {
            return true;
        }
        int compareAlarmId= (saveId==1) ? 2 : 1;
        Alarm compareAlarm=getAlarm(context.getContentResolver(),compareAlarmId);
        /**Lenovo-sw linrk1 modify begin 2014-05-22,cause Coverity CID 14539**/
        if(compareAlarm == null){
            Log.d(TAG,"------->>>getAlarm return null !!!! ");
            return false;
        }
        /**Lenovo-sw linrk1 modify end 2014-05-22,cause Coverity CID 14539**/
        if(compareAlarm.mEnabled == false) {
            return true;
        }
        int saveMinuteTime=saveHour*60+saveMinute;
        int settedMinuteTime=compareAlarm.mHour * 60 + compareAlarm.mMinutes;
        int[] saveTime=new int[7*2];
        int[] settedTime=new int[7*2];
        calculateCompareAlarmTime(saveTime,saveMinuteTime,saveDays,
            settedTime,settedMinuteTime,compareAlarm.mDaysOfWeek);
        for(int i=0;i<saveTime.length;i++){
            Log.d(TAG,"saveTime["+i+"]="+saveTime[i]+";settedTime["+i+"]="+settedTime[i]);
        }
        boolean compareResult=compareAlarmTime(saveTime,settedTime);
        Log.d(TAG,"isAlarmTimeAllowed="+compareResult);
        return compareResult;
    }
    /*Lenovo-sw wuzb1 add 2013-10-10 end, add time invalidation check */

    private static CharSequence getDay24ModeFormat() {
            String skeleton = "EHm";
            return DateFormat.getBestDateTimePattern(Locale.getDefault(), skeleton);
    }


    private static CharSequence getDay12ModeFormatWithAmPm() {
        String skeleton = "Ehma";
        return DateFormat.getBestDateTimePattern(Locale.getDefault(), skeleton);
    }
    /**Lenovo-sw linrk1 modify begin 2014-10-13 add**/
    /**
    ...
    /**Lenovo-sw linrk1 modify end 2014-10-13 add**/
}
