...
public class ImageLoader {
    ...
    private static int sDefaultImage = 0;
    ...
    public void asyncLoadBitmap2(final String url, final int width, final Context context, final ImageCallback callback) {
        ...
        if (null != bitmap) { // 内存
            ...
            Bitmap bitmaptemp = BitmapFactory.decodeResource(context.getResources(), sDefaultImage);
    ...
    private class LoadBitmapRunnable implements Runnable {
        ...
        public void run() {
            ...
            try {
                 ...
                 * getExternalStorageState())){ //String name =
                 * IOHelper.getName(url); //String ext =
                 * IOHelper.getExtension(name); String destFileName =
                 * MD5Encoder.encode(url); File dest = new
                 * File(Constants.CACHE_DIR_IMAGE, destFileName);
                 * if(!dest.exists()){ try { HttpHelper.download(url, dest); bmp
                 * = ImageHelper.loadFromFile(dest); imageCache.put(url, new
                 * SoftReference<Bitmap>(bmp)); //放入内存缓存 msg.obj = bmp;
            ...
            // if(null != bmp){
            // Log.d(TAG, "bmp.width=" + bmp.getWidth() +",bmp.height=" +
            // bmp.getHeight() + "-->" + url);
            // }else{
