...
public class DefaultHeader extends FrameLayout implements OnPullUIListener {
    ...
    public void onUIPositionChange(PullWidget frame, boolean isUnderTouch, byte status, PullViewManager ptrIndicator) {
        ...
        if (currentPos < mOffsetToRefresh && lastPos >= mOffsetToRefresh) {
            if (isUnderTouch && status == PullWidget.PTR_STATUS_PREPARE) {
    ...
    private void crossRotateLineFromTopUnderTouch(PullWidget frame) {
        if (!frame.isPullToRefresh()) {
            mTitleTextView.setVisibility(VISIBLE);
    ...
    private void crossRotateLineFromBottomUnderTouch(PullWidget frame) {
        mTitleTextView.setVisibility(VISIBLE);
        if (frame.isPullToRefresh()) {
            mTitleTextView.setText("下拉刷新");
        } else {
