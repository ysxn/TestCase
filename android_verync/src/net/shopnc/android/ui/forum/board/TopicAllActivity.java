...
public class TopicAllActivity extends ListActivity implements UpdateHandle {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		Intent intent = new Intent(TopicAllActivity.this, TopicDetailActivity.class);
		
		Topic topic = (Topic)TopicAllActivity.this.adapter.getItem(position);
		Log.d(TAG, topic.toString());
		
		intent.putExtra(Topic.TOPIC_TAG, topic);
		intent.putExtra("isreply", TopicAllActivity.this.isreply);
		intent.putExtra("ispostimage", TopicAllActivity.this.ispostimage);
	...
	private void initPullView(){
		...
		adapter = new TopicListViewAdapter(TopicAllActivity.this);
	...
	private void initPagerBar(){
		...
		Board temp = dao.get(TopicAllActivity.this.fid);
		if(temp != null){
	...
	private void loadPage(){
		...
		if(-1 == SystemHelper.getNetworkType(this)){
			pv.endUpdate(); 
			Toast.makeText(TopicAllActivity.this, "网络连接失败，请检查设备!", Toast.LENGTH_SHORT).show();
			return;
		}
			...
			public void onClick(View v) {
				if(null != myApp.getUid() && !"".equals(myApp.getUid()) 
					...
					if(subBoardMap != null){
						...
						if(b.getIspost() == 1){
							Intent intent = new Intent(TopicAllActivity.this, SendTopicActivity.class);
							intent.putExtra("fid", TopicAllActivity.this.fid);
							intent.putExtra("boardName", TopicAllActivity.this.boardName);
							intent.putExtra("ispostimage", b.getIspostimage());
							
							TopicAllActivity.this.startActivityForResult(intent, 100);
						}else{
							Toast.makeText(TopicAllActivity.this, "你没有权限在本版发帖！", Toast.LENGTH_SHORT).show();
						}
					}
				}else{//未登录先跳转到登录
					Toast.makeText(TopicAllActivity.this, "发帖请先登录！", Toast.LENGTH_SHORT).show();
					Intent intent = new Intent(TopicAllActivity.this, LoginActivity.class);
					TopicAllActivity.this.startActivityForResult(intent, 200);
				}
			}
		});
	}
		...
		public void onClick(View v) {
			...
			case R.id.btn_pager_prev:
				if(totalpage > 1 && pageno > 1){
					...
					Toast.makeText(TopicAllActivity.this, "已经是第1页了!", 0).show();
				}
				break;
			...
			case R.id.btn_pager_next:
				if(totalpage > 1 && pageno < totalpage){
					...
					Toast.makeText(TopicAllActivity.this, "已经是底页了!", 0).show();
				}
				break;
	...
	private void favorite(){
		if(null != myApp.getSid()){
			...
			if(temp != null){
				Toast.makeText(TopicAllActivity.this, "您已经收藏了!", 0).show();
			}else{
				...
				Toast.makeText(TopicAllActivity.this, "收藏成功!", 0).show();
			}
			btn_board_favorite.setBackgroundResource(R.drawable.liked);
		}else{
			Toast.makeText(TopicAllActivity.this, "请先登录后再收藏!", 0).show();
		}
	}
}
