...
//			pv.startUpdate();
			...
			btn_right.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					if (null != myApp.getUid() && !"".equals(myApp.getUid())
						...
						if (subBoardMap != null) {
							System.out.println("fidaa a a ==>"+fid);
							Board b = subBoardMap.get(Long.valueOf(fid));
							if (b.getIsreply() == 1) {
								Intent intent = new Intent(
										TopicDetailLastRepliesHtmlActivity.this,
										ReplyTopicActivity.class);
								intent.putExtra("fid",TopicDetailLastRepliesHtmlActivity.this.fid);
								intent.putExtra("tid",TopicDetailLastRepliesHtmlActivity.this.tid);
								intent.putExtra("ispostimage", b.getIspostimage());
								TopicDetailLastRepliesHtmlActivity.this
										.startActivityForResult(intent, 100);
							} else {
								Toast.makeText(TopicDetailLastRepliesHtmlActivity.this,
						...
						Intent intent = new Intent(TopicDetailLastRepliesHtmlActivity.this,
								LoginActivity.class);
						TopicDetailLastRepliesHtmlActivity.this.startActivityForResult(
								intent, 200);
					}
				}
			});
	...
	private void loadPage(final int pageno) {
		...
		if (-1 == SystemHelper.getNetworkType(this)) {
			Toast.makeText(TopicDetailLastRepliesHtmlActivity.this, "网络连接失败，请检查设备!",
...
//							adapter.setPageno(pageno);
							...
							if(datas.size()!=0 && datas!=null){
								...
								for(int i=0;i<datas.size();i++){
									...
									str.append("<tr><td style=\"padding-top:8px;\" class=\"app1\"><img src=\""+c.getAvatar()+"\"></td> <td class=\"app1_h1\"> <span>"+c.getAuthor()+"</span>发布日期:"+DateAndTimeHepler.friendly_time(TopicDetailLastRepliesHtmlActivity.this,c.getDateline() * 1000)+"</td><td align=\"right\">"+louceng+"</td></tr>");
...
//									System.out.println("hjgang22222---->"+str4.toString());
									...
									if (ConnectivityManager.TYPE_WIFI == SystemHelper
											.getNetworkType(TopicDetailLastRepliesHtmlActivity.this)
											|| !img_invisible) {
										// 显示
					                    ...
					                    public void run() { 
					                       String mage=null;
											...
											if(subBoardMap != null){
												...
												if(b2.getIsreply() == 1){
													Intent it = new Intent(TopicDetailLastRepliesHtmlActivity.this, QuoteTopicActivity.class);
													...
													TopicDetailLastRepliesHtmlActivity.this.startActivityForResult(it, 300);
												}else{
													Toast.makeText(TopicDetailLastRepliesHtmlActivity.this, "你没有权限在本版回帖！", Toast.LENGTH_SHORT).show();
												}
										}
					                    } 
					                }); 
		...
		public void onClick(View v) {
			...
			case R.id.btn_pager_prev:
				if (totalpage > 1 && pageno > 1) {
					...
					Toast.makeText(TopicDetailLastRepliesHtmlActivity.this, "已经是第1页了!",
							Toast.LENGTH_SHORT).show();
				}
				break;
			case R.id.btn_pager_next:
				if (totalpage > 1 && pageno < totalpage) {
					...
					Toast.makeText(TopicDetailLastRepliesHtmlActivity.this, "已经是底页了!",
							Toast.LENGTH_SHORT).show();
				}
				break;
