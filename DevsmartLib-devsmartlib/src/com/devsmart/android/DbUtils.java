...
public class DbUtils {
    ...
    public static String lookupSingleStringValue(SQLiteDatabase db, String columnName, String table, String selection, String[] selectionArgs,
            String defaultValue) {
        String retval = defaultValue;
     ...
     * query = "UPDATE OR REPLACE %s SET "; Iterator<Entry<String, Object>> it =
     * values.valueSet().iterator(); while(it.hasNext()){ Entry<String, Object>
     * entry = it.next(); query += String.format("%s = '%s'", entry.getKey(),
     * entry.getValue().toString()); if(it.hasNext()){ query += ", "; } } query
     * += " WHERE"; selection.rep String.format(selection, selectionArgs)); }
     */
