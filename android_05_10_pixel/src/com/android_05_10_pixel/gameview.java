...
public class gameview extends View implements Runnable {
    ...
    void DropStone(int x, int y, int stonesize, int stoneweight)
    {
        ...
        {
            // Log.i("zhuyawen", "x = " + Integer.toString(x) +" y = "
            // +Integer.toString(y) + " stonesize = " +
            // Integer.toString(stonesize) +
            // " BACKWIDTH = " + Integer.toString(BACKWIDTH) +" BACKHEIGHT = "
            // +Integer.toString(BACKHEIGHT));
            return;
        }
        for (int posx = x - stonesize; posx < x + stonesize; posx++)
        {
            for (int posy = x - stonesize; posy < x + stonesize; posy++)
            {
                if ((posx - x) * (posx - x) + (posy - y) * (posy - y) < stonesize * stonesize)
                {
                    ...
                    // Log.i("buf1[BACKWIDTH*posy + posx] = ",
                    // Short.toString(buf1[BACKWIDTH*posy + posx]) +
                    // Integer.toString(posx) + Integer.toString(posy));
                }
            }
        }
        // Log.i("zhuyawen after", "x = " + Integer.toString(x) +" y = "
        // +Integer.toString(y) + " stonesize = " + Integer.toString(stonesize)
        // +
        // " BACKWIDTH = " + Integer.toString(BACKWIDTH) +" BACKHEIGHT = "
        // +Integer.toString(BACKHEIGHT));
    ...
    {
        super.onDraw(canvas);
        // drawBitmap(int[] colors, int offset, int stride, int x, int y, int
        // width, int height, boolean hasAlpha, Paint paint)
        // Legacy version of drawBitmap(int[] colors, ...) that took ints for
        // x,y
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        switch (event.getAction())
        {
            case MotionEvent.ACTION_DOWN:
                ...
                // Log.i("zhuyawen", "event.getX() = " +
                // Float.toString(event.getX()) +" event.getY() = "
                // +Float.toString(event.getY())
                // + " event.getXPrecision() = " +
                // Float.toString(event.getXPrecision()) +
                // " event.getYPrecision() = " +
                // Float.toString(event.getYPrecision())
                // +" event.getRawX() = " +Float.toString(event.getRawX()) +
                // " event.getRawY() = " +Float.toString(event.getRawY()));
                break;
        }
