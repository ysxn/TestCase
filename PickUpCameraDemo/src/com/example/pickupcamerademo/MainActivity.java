...
//        } else 
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            } else if (mFlagActivityResume && !mFlagProximityNear && mFlagStartMonitor && Math.abs(mSensorAcceData[0]) >= 8
                    && Math.abs(mSensorAcceData[1]) <= 3 && Math.abs(mSensorAcceData[2]) <= 3) {
                mFlagStartMonitor = false;
                launchBackCamera();
            } else if (mFlagActivityResume && !mFlagProximityNear && mFlagStartMonitor && Math.abs(mSensorAcceData[0]) <= 3
                    && mSensorAcceData[1] >= 8
